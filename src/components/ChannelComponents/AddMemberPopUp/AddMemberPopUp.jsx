...
import { getMembers, addMembers, removeMember } from '../../../api/Channel/Channel';
...
export default function AddMemberPopUp({ chatInfo, setShowMemList }) {
    ...
    const addParticipants = (data) => {
        console.log(data);
        const selected = [...selectedMembers, data];
        setSelectedMembers((selectedMembers) => [...selectedMembers, data]);
        setMembers();
        fetchMembers("", selected.map((member) => member.id));
    }

    const removeParticipants = (data) => {
        console.log(data);
        const selected = selectedMembers.filter((mem) => mem.id !== data.id);
    ...
    const handleRemoveMember = async (data) => {
        const res = await removeMember({ user_id: data.user._id, channel_id: chatInfo._id })
    }
                ...
                <div>
                    <div className="mcontent zcrchmcnt" style={{ width: "500px", padding: "0px" }}>
                        <div className="zcl-win-modal1-hdr fshrink  flexC justifySB">
                            <div className="flexC">
                                <div className="font17 fontB ellips cur" onClick={() => { setAddMem(false); setSelectedMembers([]) }}>#Prezz</div>
                                    ...
                                    {superUsers.length > 0 && superUsers.map((mem, index) => {
                                        return <div key={mem._id} uid={mem._id} className="zcl-win-modal1-item">
                                            <div className="fshrink posrel contct-img bdrR100">
                                                <img className="w100 h100 bdrR100" src={mem.user.image_url.includes("https") ? mem.user?.image_url : process.env.REACT_APP_BUCKET_END_POINT + mem.user?.image_url} style={{ background: "transparent" }} />
                                            </div>
                                            <div className="mL12 flexG ellips">
                                                <div className="flexC">
                                                    <div className="font15 clr-M curP dIB ellips">{mem.user?.user_id === user.user_id ? "You" : mem.user?.first_name + " " + mem.user?.last_name}</div>
                                                </div>
                                        ...
                                        return <div key={mem._id} uid={mem._id} className="zcl-win-modal1-item">
                                            ...
                                            <span id="rolechange60016689060" uid="60016689060" className="tooltip rolechange-icn mL12 zcl-icon zcl-icon--plain zcf-moreV" purpose="changerole" currentrole="1" sameorguser="true">
                                                ...
                                                <div className='zcl-menu-wrap '>
                                                    ...
                                                    {/* <div className='ellips zcl-menu-item'>Assign as moderators</div> */}
                                                    {/* <div className='ellips zcl-menu-item'>Assign as moderators</div> */}
                                                    <div className='ellips zcl-menu-item remove' onClick={() => handleRemoveMember(mem)}>Remove</div>
                        ...
                        {addMem && <div className="zcrchtlt flex-col">
                            <div
                                className="flexG zcteammember-list posl ps-container ps-theme-default ps-active-y">
                                <div id="participants" style={{ overflow: "auto", height: "100%", overflowX: "hidden", scrollBehavior: "smooth" }}>
                                    {!members ? <CircleLoader></CircleLoader> : members.length <= 0 && <p style={{ textAlign: "center", color: "gray" }}>No user Found</p>}
                                    {members && members.map((mem) => {
                                        return <div uid={mem.id} key={mem.id} className="cur zcl-win-modal1-item" onClick={() => addParticipants(mem)}>
                                            <div className="fshrink posrel contct-img bdrR100">
                                                <img className="w100 h100 bdrR100" src={process.env.REACT_APP_BUCKET_COMP_END_POINT + mem.user?.image_url} style={{ background: "transparent" }} />
                                            </div>
                                            <div className="mL12 flexG ellips">
                                                <div className="flexC"><div className="font15 clr-M curP dIB ellips">{user.id === mem.id ? "You" : mem.user.first_name + " " + mem.user.last_name}</div></div>
                                    ...
                                    {selectedMembers.length > 0 &&
                                        <div className='flexC justifySB' style={{ position: "absolute", bottom: "0", padding: "20px" }}>
                                            {selectedMembers.map((mem) => {
                                                console.log("selected list", mem);
                                                return <div key={mem.id} className="add_part_img cur zcl-win-modal1-footr fshrink" style={{ position: "relative" }}>
                                                    <div className='' style={{ width: "38px", height: "38px", borderRadius: "50%", overflow: "hidden" }} >
                                                        <img width="100%" height="100%" src={process.env.REACT_APP_BUCKET_COMP_END_POINT + mem.user.image_url}></img>
                                                    </div>
                                                    <div className="posabs cancel_user" title={mem.user.first_name + " " + mem.user.last_name ?? ""} onClick={() => removeParticipants(mem)}>
                                    ...
                                    <button className="btn btn-primary" style={{ position: "absolute", bottom: "20px", right: "20px" }} onClick={addChannelMembers}>Add</button>
                                </div>
                            </div>
                        </div>}
